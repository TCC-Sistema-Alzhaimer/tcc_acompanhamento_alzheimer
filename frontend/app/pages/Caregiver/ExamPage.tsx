...
function ExamPage() {
  ...
  const handleFilterExams = (status: string) => {
    if (status === "all") {
      ...
      setUserExams(
        exams.filter(
          (exam) => exam.patient_id === userId && exam.status === status
        )
      );
    }
  };
    ...
    return (
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-col gap-3 border border-gray-100 rounded-lg p-4 h-full mt-5">
          ...
          <Button type="button" onClick={() => setOpenAttachExam(true)}>
            {" "}
            Anexar Exames{" "}
          </Button>
        </div>
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <div>
        ...
        <Button
          type="button"
          onClick={() => handleFilterExams("completed")}
          className="mr-2"
        >
          Concluídos
        </Button>
        <Button
          type="button"
          onClick={() => handleFilterExams("pending")}
          className="mr-2"
        >
          Pendentes
        </Button>
      ...
      <div className="flex flex-col gap-3 border border-gray-100 rounded-lg p-4 h-full">
        {userExams.length > 0 ? (
          userExams.map((exam) => (
            <List.ItemRoot>
              <Link
                to={generatePath(ROUTES.CAREGIVER.EXAMINATION, {
