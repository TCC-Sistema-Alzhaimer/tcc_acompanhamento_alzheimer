...
// #1234567890
// Estados que permitem criar conclusão
const ALLOWED_STATUS_FOR_CONCLUSION = [
  "REQUESTED", // Solicitado
  "SCHEDULED", // Agendado
  "IN_PROGRESS", // Em Andamento
  "COMPLETED", // Concluído
  "PENDING_RESULT", // Aguardando Resultado
  // "CANCELLED",    // Cancelado - não permite conclusão
];
...
export function ExamDetail({ examId, onBack }: ExamDetailProps) {
  ...
  const canCreateConclusion =
    exam &&
    ALLOWED_STATUS_FOR_CONCLUSION.includes(
      exam.examStatusId?.toUpperCase() || ""
    );
  ...
  const handleGoToConclusion = () => {
    if (exam) {
      navigate(ROUTES.DOCTOR.CONCLUSION, {
        state: {
          defaultPatientId: exam.patientId,
  ...
  const handleViewConclusion = (conclusionId: number) => {
    if (exam) {
      navigate(
        `${ROUTES.DOCTOR.CONCLUSION}?tab=list&conclusionId=${conclusionId}`,
        {
          state: {
            defaultPatientId: exam.patientId,
          },
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      <div className="p-6">
        ...
        {/* Seção de Documentos/Resultados */}
        <div className="mt-6 pt-6 border-t border-gray-200">
              ...
              {conclusions.map((conclusion) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleViewConclusion(conclusion.id)}
                        className="flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full border bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100 transition-colors cursor-pointer"
                      >
                        <ExternalLink size={12} />
                        Ver Conclusão
                      </button>
                      <span className="text-xs text-gray-600">
                        {new Date(conclusion.createdAt).toLocaleDateString(
                          "pt-BR"
                        )}
                      </span>
                    </div>
        ...
        {/* Botão de adicionar conclusão no final */}
        {canCreateConclusion && (
          <div className="mt-6 pt-6 border-t border-gray-200 flex justify-end">
            <button
              onClick={handleGoToConclusion}
              className="flex items-center gap-2 px-4 py-2 bg-teal-600 text-white font-medium rounded-lg hover:bg-teal-700 transition-colors cursor-pointer"
            >
              <PlusCircle size={18} />
              Adicionar Conclusão
            </button>
